...
(function () {
  ...
  const CFG = {
    ...
    contacto: 'compras@skysense.com.mx',
    ...
    if (ultimoError) {
      return {
        html: md('No pude consultar la base de conocimiento en este momento ' +
          ...
          'y mientras tanto escribe tu duda a **' + CFG.contacto + '**.\n\n' +
  ...
  window.SkyAsistente = {
    ...
    diagnostico: async function () {
      const r = { perfil: perfil, rol: rol, modulo: modulo, sb: !!window.sb };
      try {
        const b = await sb.rpc('asist_buscar',
