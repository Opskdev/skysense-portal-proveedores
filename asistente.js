...
(function () {
  ...
  /* Suma agrupada por moneda. Sumar MXN y USD en un solo número produce una
     cifra que no significa nada, y es de las que alguien copia a un correo. */
  function sumaPorMoneda(lista) {
    const tot = {};
    (lista || []).forEach(f => {
      const mo = f.moneda || 'MXN';
      tot[mo] = (tot[mo] || 0) + Number(f.total || 0);
    });
    const partes = Object.keys(tot).map(mo => money(tot[mo], mo) + ' ' + mo);
    return partes.join(' + ');
  }
  ...
  /* list-style explícito: el CSS global del portal resetea las listas y las
     numeradas se reiniciaban visualmente a mitad del bloque. */
  .sa-msg ul,.sa-msg ol{margin:6px 0 8px 20px;padding:0}
  .sa-msg ul{list-style:disc outside}
  .sa-msg ol{list-style:decimal outside}
  .sa-msg li{margin-bottom:4px;display:list-item;list-style-position:outside}
